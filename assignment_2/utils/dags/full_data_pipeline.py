...
with DAG(
    ...
    description='Data pipeline run once - process all raw data in one go across many snapshot_date',
    ...
    bronze_layer = BashOperator(
        ...
        bash_command=(
            f"python {SCRIPTS}/data_processing/process_bronze_full_batch.py"
        ),
    ...
    silver_layer = BashOperator(
        ...
        bash_command=(
            # Use printf without extra quotes to write XCom output to a temp file
            "printf '%s' '{{ ti.xcom_pull(task_ids=\"bronze_layer\") | tojson }}' > /tmp/bronze_manifest.json && "
            f"python {SCRIPTS}/data_processing/process_silver_full_batch.py --bronze_manifest /tmp/bronze_manifest.json"
        ),
    ...
    gold_layer = BashOperator(
        ...
        bash_command=(
            ...
            f"python {SCRIPTS}/data_processing/process_gold_full_batch.py --silver_manifest /tmp/silver_manifest.json"
        ),
        execution_timeout=timedelta(hours=1),
    )
